...
# ==========================================
# QR Code Decoder (Native OpenCV - No libzbar dependency required)
# ==========================================
qr_detector = cv2.QRCodeDetector()

def decode_qr(image_bytes):
    """
    Decodes QR code from image bytes using native OpenCV C++ detector.
    Does NOT depend on external OS libraries (like libzbar), making it 100% stable on Railway/Linux.
    """
    try:
        ...
        success, decoded_info, points, _ = qr_detector.detectAndDecodeMulti(img)
        if success and decoded_info:
            valid_urls = [info.strip() for info in decoded_info if info and info.strip()]
            if valid_urls:
                return valid_urls[0]
        ...
        # Optional pyzbar fallback if library is available on host
        try:
...
def extract_id_name(image_bytes):
    """
    Extracts user ID/Name from the top-left quadrant of an image.
    Downscales image & uses lightweight OCR to conserve memory & max out speed.
    """
    try:
        image = Image.open(io.BytesIO(image_bytes))
        width, height = image.size
        
        # Crop to top-left quadrant (name is located here, watermark top-right)
        cropped = image.crop((0, 0, int(width * 0.5), int(height * 0.5)))
        
        # Downscale for ultra-fast OCR if image is very large (max width 400px)
        c_w, c_h = cropped.size
        if c_w > 400:
            ratio = 400.0 / c_w
            cropped = cropped.resize((400, int(c_h * ratio)), Image.Resampling.LANCZOS)
        ...
        if USE_RAPIDOCR and rapid_engine:
            ocr_res, _ = rapid_engine(fast_image_bytes)
            if ocr_res:
                ocr_res.sort(key=lambda x: x[0][0][1])
                for item in ocr_res:
                    results.append((item[1], float(item[2])))
        elif easyocr_reader:
            ocr_res = easyocr_reader.readtext(fast_image_bytes, paragraph=False)
            ocr_res.sort(key=lambda x: x[0][0][1])
            for item in ocr_res:
                results.append((item[1], float(item[2])))
        
        # UI filter words (lowercase)
        skip_words = [
        ...
        for text, prob in results:
            ...
            # Clean LV badges & OCR noise artifacts
...
def process_batch(chat_id, user_id, format_type, images):
    ...
    # 2. Process images (OCR + Native OpenCV QR decoding)
