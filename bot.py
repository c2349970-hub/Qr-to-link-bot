...
def run_ocr_on_crop(cropped_img):
    """Helper to run RapidOCR / EasyOCR on a cropped PIL Image"""
    results = []
    try:
        if USE_RAPIDOCR and rapid_engine:
            # Convert PIL RGB -> BGR numpy array required by RapidOCR/PaddleOCR ONNX
...
def extract_id_name(image_bytes):
    ...
    try:
        ...
        # Test crops in sequence: Top-Left (75% W x 50% H), Top-Half (100% W x 65% H), Full Image
        crops_to_test = [
            image.crop((0, 0, int(width * 0.75), int(height * 0.5))),
            image.crop((0, 0, width, int(height * 0.65))),
            image
        ]
        
        results = []
        for crop in crops_to_test:
            results = run_ocr_on_crop(crop)
            if results:
                break
                
        if not results:
            print("❌ [OCR LOG] No text detected in image crops or full image")
            return "Unknown"

        print(f"🔍 [OCR LOG] Detected {len(results)} text items in image")
        ...
        for text, prob in results:
            ...
            print(f"  ➜ [OCR ITEM] Raw: '{raw_text}' | Cleaned: '{cleaned_text}' (score: {prob:.2f})")
            
            # If valid username remains after stripping UI text, return it!
            if cleaned_text and len(cleaned_text) >= 2:
                print(f"✅ [OCR MATCH] Selected ID Name: '{cleaned_text}'")
                return cleaned_text
        
        raw_texts = [r[0] for r in results]
        print(f"⚠️ [OCR LOG] All text items were filtered out. Raw detected: {raw_texts}")
        return "Unknown"
