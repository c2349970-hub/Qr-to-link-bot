...
# ==========================================
# Lightweight OCR Engine Setup (RapidOCR ONNX)
# ==========================================
USE_RAPIDOCR = False
rapid_engine = None
easyocr_reader = None
...
try:
    ...
    USE_RAPIDOCR = True
    print("RapidOCR ONNX Engine Ready! (RAM footprint: ~80MB)")
except Exception as e:
    print(f"RapidOCR ONNX failed: {e}. Trying EasyOCR fallback...")
    try:
        import easyocr
        easyocr_reader = easyocr.Reader(['ch_sim', 'en'], gpu=False, quantize=True)
        print("Optimized EasyOCR Model Ready!")
    except Exception as err:
        print(f"EasyOCR fallback error: {err}")

def run_ocr_on_crop(cropped_img):
    """Helper to run RapidOCR / EasyOCR on a cropped PIL Image with BGR conversion"""
    results = []
    try:
        if USE_RAPIDOCR and rapid_engine:
            # Convert PIL RGB -> BGR numpy array required by RapidOCR/PaddleOCR ONNX models
            rgb_np = np.array(cropped_img)
            bgr_np = np.ascontiguousarray(rgb_np[:, :, ::-1])
            
            ocr_res, _ = rapid_engine(bgr_np)
            if ocr_res:
                ocr_res.sort(key=lambda x: x[0][0][1])
                for item in ocr_res:
                    results.append((item[1], float(item[2])))
        elif easyocr_reader:
            img_byte_arr = io.BytesIO()
            cropped_img.save(img_byte_arr, format='JPEG', quality=95)
            fast_bytes = img_byte_arr.getvalue()
            ocr_res = easyocr_reader.readtext(fast_bytes, paragraph=False)
            if ocr_res:
                ocr_res.sort(key=lambda x: x[0][0][1])
                for item in ocr_res:
                    results.append((item[1], float(item[2])))
    except Exception as e:
        print(f"  OCR Engine Exception: {e}")
    return results

def extract_id_name(image_bytes):
    """
    Extracts user ID/Name from an image.
    ...
    try:
        ...
        # Primary Crop: Top-left area (80% width x 50% height)
        cropped_1 = image.crop((0, 0, int(width * 0.8), int(height * 0.5)))
        results = run_ocr_on_crop(cropped_1)
        
        # Secondary Crop Fallback: Top 70% of entire image if primary crop yielded no text
        if not results:
            cropped_2 = image.crop((0, 0, width, int(height * 0.7)))
            results = run_ocr_on_crop(cropped_2)

        # Final Fallback: Run on full image if crops yield no text
        if not results:
            results = run_ocr_on_crop(image)
            
        if not results:
            print("  OCR: No text detected in image crops or full image")
            return "Unknown"
        ...
        for text, prob in results:
            ...
            # 3. Clean misread badge icon artifacts (@凹, 凹, Q, 0, O) and trailing single characters
            cleaned_text = re.sub(r'\s*[@]?[凹Q0O]\s*$', '', cleaned_text)
            ...
            if cleaned_text and len(cleaned_text) >= 2:
                print(f"  OCR detected ID name: '{cleaned_text}' (raw: '{raw_text}', confidence: {prob:.2f})")
                return cleaned_text
        
        raw_texts = [r[0] for r in results]
        print(f"  OCR Debug (No valid name after filter): Raw texts -> {raw_texts}")
        return "Unknown"
...
def process_batch(chat_id, user_id, format_type, images):
    ...
    processing_msg = bot.send_message(chat_id, "⚡ Downloading and processing images with Lightweight AI... Please wait.")
    ...
    # 2. Process images (OCR + Standalone ZXing C++ QR decoding)
    for downloaded_file, file_id in downloads:
        if not downloaded_file:
            continue
            
        try:
            # Extract ID Name with Lightweight OCR
