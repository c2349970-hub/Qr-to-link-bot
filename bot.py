...
print("--------------------------------------------------")
print("OCR Engine  : Tesseract (chi_sim + eng) — Zero cv2 dependency, ~5MB footprint")
print("QR Engine   : zxing-cpp — Zero libzbar/libxcb dependency")
print("--------------------------------------------------")

bot = telebot.TeleBot(TOKEN)

# ==========================================
# QR Code Decoder (zxing-cpp)
# ==========================================
def decode_qr(image_bytes):
    """
    Decodes QR code using Google ZXing C++ engine.
    Zero external OS library dependencies — 100% stable on Railway/Docker.
    """
    try:
...
# ==========================================
# ID Name Extraction (Tesseract OCR)
# No cv2, No PyTorch, No libxcb — Pure C binary
# ==========================================

# Tesseract config: use both Chinese Simplified + English, PSM 6 (assume uniform block of text)
TESS_CONFIG = r'--oem 1 --psm 6 -l chi_sim+eng'

def extract_id_name(image_bytes):
    """
    Extracts user ID/Name from an image using Tesseract OCR.
    Zero cv2 dependency — uses only Pillow + pytesseract (C binary).
    Strips UI text elements (e.g. 'posted a comment', 'shared via') from lines.
    """
    try:
        image = Image.open(io.BytesIO(image_bytes)).convert("RGB")
        width, height = image.size

        # Primary crop: top-left (75% width x 50% height)
        crops = [
            image.crop((0, 0, int(width * 0.75), int(height * 0.5))),
        ...
        raw_text = ""
        for crop in crops:
            raw_text = pytesseract.image_to_string(crop, config=TESS_CONFIG).strip()
            if raw_text:
                break

        if not raw_text:
            print("❌ [Tesseract] No text detected in image")
            return "Unknown"

        lines = [l.strip() for l in raw_text.splitlines() if l.strip()]
        print(f"🔍 [Tesseract] Detected {len(lines)} lines: {lines}")
        ...
        for line in lines:
            ...
            # Skip status bar elements (clock, battery, network)
            if re.match(r'^\d{1,2}:\d{2}$', raw_line) or re.match(r'^\d+%?$', raw_line):
                continue
            if raw_line.upper() in ['4G', '5G', 'LTE', 'WIFI', 'HD']:
                continue
            ...
            print(f"  ➜ [Tesseract] Raw: '{raw_line}' | Cleaned: '{cleaned}'")

            if cleaned and len(cleaned) >= 2:
                print(f"✅ [Tesseract MATCH] ID Name: '{cleaned}'")
                return cleaned

        print(f"⚠️ [Tesseract] All lines filtered. Raw: {lines}")
        return "Unknown"
...
def process_batch(chat_id, user_id, format_type, images):
    ...
    processing_msg = bot.send_message(chat_id, "⚡ Processing images... Please wait.")
    processed_images = []
    ...
    for downloaded_file, file_id in downloads:
        ...
        try:
            ...
            processed_images.append({'url': url, 'id_name': id_name, 'file_id': file_id})
        except Exception as e:
        ...
        if format_type == 'with':
            for name, imgs in grouped.items():
                for i in range(0, len(imgs), 10):
                    chunk = imgs[i:i+10]
                    links = [f"<b>{name}</b>\n"] + [img['url'] for img in chunk]
                    caption = "\n".join(links)
                    media = [InputMediaPhoto(chunk[0]['file_id'], caption=caption, parse_mode="HTML")]
                    for img in chunk[1:]:
                        media.append(InputMediaPhoto(img['file_id']))
                    bot.send_media_group(chat_id, media)
        elif format_type == 'without':
            for name, imgs in grouped.items():
                text = f"<b>{name}</b>\n\n" + "\n".join(img['url'] for img in imgs)
