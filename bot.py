...
def extract_id_name(image_bytes):
    try:
        ...
        # RapidOCR takes a numpy array directly
...
def download_image(file_id):
    try:
        ...
        # Explicit timeout so a stuck network call fails loudly instead of hanging forever
...
def process_batch(chat_id, user_id, format_type, images):
    ...
    # (RapidOCR is ONNX-based and thread-safe, but sequential keeps memory predictable)
    total = len(downloads)
    for idx, (downloaded_file, file_id) in enumerate(downloads, 1):
        print(f"  Processing image {idx}/{total} ({file_id})...")
