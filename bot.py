...
def extract_id_name(image_bytes):
    """
    Extracts user ID/Name from top quadrant of an image using RapidOCR / EasyOCR.
    Converts PIL image to numpy array for 100% reliable RapidOCR detection.
    """
    try:
        image = Image.open(io.BytesIO(image_bytes)).convert("RGB")
        width, height = image.size
        
        # Crop top-left region: 60% width x 45% height (captures username safely)
        cropped = image.crop((0, 0, int(width * 0.6), int(height * 0.45)))
        
        # Downscale to max width 600px if original crop is very large (preserves text resolution)
        c_w, c_h = cropped.size
        if c_w > 600:
            ratio = 600.0 / c_w
            cropped = cropped.resize((600, int(c_h * ratio)), Image.Resampling.LANCZOS)
        
        results = []
        if USE_RAPIDOCR and rapid_engine:
            # Pass numpy array directly to RapidOCR for 100% reliable ONNX decoding
            cropped_np = np.array(cropped)
            ocr_res, _ = rapid_engine(cropped_np)
            ...
            cropped.save(img_byte_arr, format='JPEG', quality=95)
        ...
        # Log detected raw text for debugging if no valid ID was found
        if results:
            raw_texts = [r[0] for r in results]
            print(f"  OCR Debug (No match): Raw detected texts -> {raw_texts}")
        else:
            print("  OCR: No text detected by engine in cropped area")
